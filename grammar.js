...
module.exports = grammar({
  ...
  conflicts: $ => [
    ...
    [$._unannotated_type, $._variable_declarator_id],
    [$.generic_type, $.primary_expression],
    [$.statement, $.expression],
    // Only conflicts in switch expressions
    [$.lambda_expression, $.primary_expression],
    [$._closure_parameter, $.lambda_expression],
    [$.inferred_parameters, $.primary_expression],
    [$._func_parameters_identifier, $._unannotated_type, $.primary_expression],
    [$.class_literal, $.field_access],
    [$.primary_expression, $.expression],
    [$.groovy_closure, $.block, $.element_value_array_initializer],
    [$.groovy_closure, $.block],
    [$.groovy_closure, $.element_value_array_initializer],
    [$._closure_parameter, $.primary_expression],
    [$.lambda_expression, $.groovy_as_expression],
    // `multiple_assignment`, `inferred_parameters`
    [$.multiple_assignment, $.inferred_parameters],
  ],
  ...
  rules: {
    ...
    triple_single_quoted_string: $ => token(
      seq(
        "'''",
        optional(/([^']|('[^'])|(''[^']))+/),
        "'''"
    ...
    triple_double_quoted_string: $ => token(
      seq(
        '"""',
        optional(/([^"]|("[^"])|(""[^"]))+/),
        '"""'
    ...
    // Groovy List Map
    _groovy_element_value_pair: $ => seq(
      field('key', choice($.string_literal ,$.identifier)),
      ':',
      field('value', $._groovy_element_value)
    ),

    _groovy_element_value: $ => prec(PREC.ELEMENT_VAL, choice(
      $.expression,
      $._annotation
    )),

    groovy_list: $ => prec.right(seq(
      '[',
      commaSep1($._groovy_element_value),
      optional(','),
      ']'
    )),
    groovy_map: $ => prec.right(seq(
      '[',
      commaSep1($._groovy_element_value_pair),
      optional(','),
      ']'
    )),

    // groovy closure
    _closure_parameter: $ => seq(
      optional( commaSep(
    ...
    expression: $ => choice(
      ...
      $.groovy_closure,
      $.groovy_print,
      $.groovy_as_expression,
      $.groovy_range_expression
    ),

    // range operater
    groovy_range_expression: $ => prec(PREC.SHIFT,seq(
      $.primary_expression,
      "..",
      $.primary_expression,
    )),
    ...
    groovy_print: $ => prec.left(PREC.REL,seq(
      choice("println", "print"), $.expression
    )),
    ...
    _func_parameters_identifier: $ => seq(
      optional( field('type', $._unannotated_type) ), $.identifier
    ),

    func_parameters: $ => seq(
      '(',
      commaSep(choice($._func_parameters_identifier, $._reserved_identifier)),
      ')'
    ),

    groovy_def_func: $ => seq(
      choice("def", $._unannotated_type, seq("def", $._unannotated_type) ), 
      $.identifier, $.func_parameters,"{", 
      repeat($.statement), 
      "}"
    ),
